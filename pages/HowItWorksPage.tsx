...
const HowItWorksPage: React.FC = () => {
  ...
  return (
    <div className="pt-32 pb-24 relative overflow-hidden">
      ...
      {/* Impact Reports Section - Highly Refined for Mobile Viewports */}
      <section className="max-w-7xl mx-auto px-6 py-20 md:py-48 bg-transparent relative">
        <div className="text-center mb-20 md:mb-40">
          <div className="inline-flex items-center gap-3 mb-6">
            <span className="w-8 h-[1px] bg-indigo-500"></span>
            <span className="text-indigo-400 font-black uppercase tracking-[0.4em] text-[9px] md:text-[10px]">Verification Module</span>
            <span className="w-8 h-[1px] bg-indigo-500"></span>
          </div>
        ...
        <div className="grid gap-12 md:gap-24 lg:gap-32">
          {caseStudies.map((study, i) => (
            <div key={study.id} className="group relative">
              <div className="absolute -inset-2 bg-gradient-to-br from-indigo-500/10 via-pink-500/10 to-transparent rounded-[2.5rem] md:rounded-[5rem] blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-1000"></div>
              
              <div className="glass-effect rounded-[2rem] sm:rounded-[3rem] md:rounded-[5rem] p-6 sm:p-10 md:p-16 lg:p-20 border border-white/5 relative z-10 grid lg:grid-cols-12 gap-10 md:gap-16 overflow-hidden">
                {/* Scaled Watermark for Mobile */}
                <div className="absolute -top-4 -right-4 sm:-top-10 sm:-right-10 p-4 sm:p-8 text-white/[0.02] text-[6rem] sm:text-[14rem] md:text-[18rem] font-black italic select-none pointer-events-none -rotate-12 group-hover:text-white/[0.05] transition-all duration-1000">
                  {i + 1}
                </div>

                <div className="lg:col-span-7 space-y-8 sm:space-y-12 md:space-y-16 relative z-10">
                  <div>
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-indigo-400 text-[8px] md:text-[9px] font-black uppercase tracking-[0.3em] mb-6">
                      <span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span>
                      Case Study: {study.type}
                    </div>
                    <h4 className="text-2xl sm:text-3xl md:text-5xl font-black text-white mb-6 tracking-tighter leading-tight md:leading-[1.1]">
                      {study.title}
                    </h4>
                    
                    <div className="grid sm:grid-cols-2 gap-6 md:gap-12">
                      <div className="space-y-2 md:space-y-4">
                        <h5 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-rose-500 flex items-center gap-2 md:gap-3">
                          <i className="fa-solid fa-triangle-exclamation"></i>
                          The Friction
                        </h5>
                        <p className="text-slate-400 text-xs sm:text-sm md:text-base leading-relaxed font-medium">{study.challenge}</p>
                      </div>
                      <div className="space-y-2 md:space-y-4">
                        <h5 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500 flex items-center gap-2 md:gap-3">
                          <i className="fa-solid fa-microchip"></i>
                          The Protocol
                        </h5>
                        <p className="text-slate-400 text-xs sm:text-sm md:text-base leading-relaxed font-medium">{study.solution}</p>
                      </div>
                    </div>
                  </div>

                  <div className="relative pl-4 sm:pl-10 border-l-2 border-indigo-500/20 italic group-hover:border-indigo-500/50 transition-colors duration-700">
                    <p className="text-base sm:text-lg md:text-xl text-slate-300 leading-relaxed mb-4 font-medium">
                      "{study.quote}"
                    </p>
                    <div className="flex items-center gap-2">
                      <div className="w-1 h-px bg-indigo-500"></div>
                      <p className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400">
                        {study.client}
                      </p>
                    </div>
                  </div>
                ...
                <div className="lg:col-span-5 flex flex-col gap-6 md:gap-8 h-full justify-center relative z-10">
                   <div className="bg-[#111827]/80 rounded-[2rem] sm:rounded-[3rem] md:rounded-[4rem] p-6 sm:p-10 md:p-12 border border-white/5 flex flex-col items-center text-center group-hover:border-indigo-500/30 transition-all duration-700 shadow-3xl">
                      <span className="text-[9px] md:text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] mb-4">{study.stats.label}</span>
                      <h5 className="text-4xl sm:text-6xl md:text-7xl font-black text-white tracking-tighter group-hover:scale-105 transition-transform duration-700">{study.stats.main}</h5>
                      <div className="w-10 sm:w-12 h-1 bg-gradient-to-r from-indigo-500 to-pink-500 mt-4 sm:mt-8 rounded-full"></div>
                   </div>

                   <div className="glass-effect rounded-[1.5rem] sm:rounded-[2.5rem] md:rounded-[3rem] p-6 sm:p-10 border border-white/10 space-y-4">
                     <div className="flex justify-between items-center mb-2">
                        <span className="text-[8px] font-black uppercase tracking-widest text-slate-600">Verification Log</span>
                     ...
                     {study.results.map((res, k) => (
                        <div key={k} className="flex items-center gap-3 group/item">
                          <div className="w-5 h-5 sm:w-7 sm:h-7 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-500 text-[8px] sm:text-[10px] border border-emerald-500/20 group-hover/item:bg-emerald-500 group-hover/item:text-white transition-all duration-500 flex-shrink-0">
                            <i className="fa-solid fa-check"></i>
                          </div>
                          <span className="text-white font-bold tracking-tight text-xs sm:text-base">{res}</span>
                        </div>
                      ))}
                   </div>
                </div>
